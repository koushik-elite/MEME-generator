...
var VintageJS = function(originalImage, opts, effect) {
  ...
  vintage = function() {
	...
	Filters.convolute = function(pixels, weights, opaque) {
	 ...
	 // Treshold
	  if (!!_effect.threshold) {
		//console.info(_imageData.length)
		  for (var i=0; i<_imageData.length; i+=4) {
			var r = _imageData[i];
			var g = _imageData[i+1];
			var b = _imageData[i+2];
			var v = (0.2126*r + 0.7152*g + 0.0722*b >= _effect.threshold) ? 255 : 0;
			_imageData[i] = _imageData[i+1] = _imageData[i+2] = v
		  }
		  
		if(_effect.patternpop == 1){
			for (var i=0; i<_imageData.length; i+=4) {
				if (_imageData[i] > 250) {
					_imageData[i] = 0;
					_imageData[i + 1] = 255;
	...
	// posterize
	  if (!!_effect.posterize) {
		console.info(_effect.posterize);
		for (var i=0; i<_imageData.length; i+=4) {
			if (_imageData[i] > 250) {
				_imageData[i] = 0;
				_imageData[i + 1] = 255;
	  ...
	  // negate
	  if (!!_effect.negate) {
		//console.info(_effect.negate);		
		for(var i = 0; i < _imageData.length; i += 4) {
		  // red
		  _imageData[i] = 255 - _imageData[i];
		  // green
		  _imageData[i + 1] = 255 - _imageData[i + 1];
		  // blue
		  _imageData[i + 2] = 255 - _imageData[i + 2];
		}
      }
